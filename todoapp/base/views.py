...
def tasks(request):
    today = date.today()
    pending_tasks = Tasks.objects.filter(completed=False, deadline__gte=today)
    completed_tasks = Tasks.objects.filter(completed=True)
    expired_tasks = Tasks.objects.filter(completed=False, deadline__lt=today)
...
def completed(request, task_id):
    task_instance = get_object_or_404(Tasks, pk=task_id)
